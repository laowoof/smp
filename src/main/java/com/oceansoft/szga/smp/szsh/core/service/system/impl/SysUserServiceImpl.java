...
import com.oceansoft.szga.smp.szsh.core.dto.system.user.FindUserDTO;
...
import com.oceansoft.szga.smp.szsh.core.dto.system.user.UserUpdateDTO;
...
import com.oceansoft.szga.smp.szsh.core.mapper.system.SysUserMapper;
import com.oceansoft.szga.smp.szsh.core.service.global.ShiroService;
import com.oceansoft.szga.smp.szsh.core.service.system.*;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    @Override
    public SysUser findUserByNameNoRole(String name){
        SysUser user = this.selectOne(new EntityWrapper<SysUser>().eq("username",name));
        return user;
    }

    @Override
    public SysUser findUserByIdNoRole(String id){
    ...
    @Override
//    @Cacheable(value = "getAllPermissionTag",keyGenerator="keyGenerator")
    public List<String> getAllPermissionTag(String id) {
        List<SysRole> allRoleByUserId = roleService.findAllRoleByUserId(id, true);
        List<String> permissions = new LinkedList<>();
        for (SysRole sysRole : allRoleByUserId) {
            if(sysRole.getResources()!=null && sysRole.getResources().size()>0){
                sysRole.getResources().forEach(s-> permissions.add(s.getPermission()));
    ...
    public Page<SysUserVO> getAllUserBySplitPage(FindUserDTO findUserDTO) {
        ...
        wrapper.like("name",findUserDTO.getName()).or().like("username",findUserDTO.getName());
        wrapper.orderBy("create_date",findUserDTO.getAsc());
        wrapper.orderBy("USERNAME",!findUserDTO.getAsc());
        ...
        userPage.getRecords().forEach(v->{
            ...
            //获取年龄
            vo.setAgeInt(this.getAgeByBirth(vo.getAge()));
    ...
    private static int getAgeByBirth(Date birthDay){
        ...
        if (cal.before(birthDay)) { //出生日期晚于当前时间，无法计算 返回0
            return age;
        }
    ...
    public void add(UserAddDTO addDTO) {
        ...
        try {
            ...
            //cmUsercaseService.updateUserCase(findUser.getId(),findUser.getCases());
        }catch (Exception e){
    ...
    public void updateUserRole(SysUser user) {
        try {
            if(user.getRoles()!=null && user.getRoles().size()>0){
                userRoleService.delete(new EntityWrapper<SysUserRole>().eq("userid",user.getId()));
                user.getRoles().forEach(v-> {
                    if(v!=null) {
                        userRoleService.insert(SysUserRole.builder()
                                .userid(user.getId())
                                .rid(v.getId()).build());
                    }
                });
